...
const contentTypeKey model.ParaKey = "accepted_content_type"

func (joint ParsePDFJoint) Name() string {
	return "parse_pdf"
}

func (joint ParsePDFJoint) Process(context *model.Context) error {
	...
	if snapshot.Title == "" && len(snapshot.File) > 0 {
		snapshot.Title = util.NoWordBreak(util.XSSHandle(snapshot.File))
	}
